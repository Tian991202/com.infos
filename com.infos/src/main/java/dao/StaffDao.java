...
//        System.out.println(st.selectUpdateStaff("小红"));
//   }

    //修改员工信息
    public boolean updateStaff(String staffName,String staffSex,String staffAge,String staffDuty,String staffWage){
        try{
            String sql="update staff set staffName='"+staffName+"',staffSex='"+staffSex+"',staffAge='"+staffAge+"' ,staffDuty='"+staffDuty+"',staffWage='"+staffWage+"'where staffName='"+staffName+"'";
            st=getStatement();
    ...
    public boolean deleteStaff(String staffName){
        try{
            String sql="delete from staff where staffName="+staffName;
            st=getStatement();
            st.executeUpdate(sql);
            return true;
        }catch (Exception e){
